...
public class JailManager {
    ...
    private final Map<UUID, Integer> individualTasks = new ConcurrentHashMap<>();
    ...
    public void initialize() {
        ...
        if (isSurvival()) {
            ...
            loadJailConfig();
            loadJailData();
            // Changed: Check less frequently (every 30 seconds instead of 10)
            Bukkit.getScheduler().runTaskTimer(plugin, this::checkJailTimes, 20L, 20L * 30);
                ...
                if (playersSection != null) {
                    for (String uuidString : playersSection.getKeys(false)) {
                        try {
                            ...
                            scheduleIndividualReleaseTask(uuid, releaseTime);
    ...
    private void scheduleIndividualReleaseTask(UUID uuid, long releaseTime) {
        long currentTime = System.currentTimeMillis();
        long delayTicks = Math.max(1, (releaseTime - currentTime) / 50); // Convert to ticks
        ...
        if (delayTicks > 0) {
            int taskId = Bukkit.getScheduler().runTaskLater(plugin, () -> {
                ...
                individualTasks.remove(uuid);
            }, delayTicks).getTaskId();

            individualTasks.put(uuid, taskId);
        }
    }
    ...
    private void checkJailTimes() {
        long currentTime = System.currentTimeMillis();
        jailedPlayers.entrySet().removeIf(entry -> currentTime >= entry.getValue().releaseTime());
    }
    ...
    public boolean jailPlayer(UUID uuid, long durationSeconds, String reason) {
        ...
        Integer existingTask = individualTasks.remove(uuid);
    ...
    public boolean unjailPlayer(UUID uuid) {
        ...
        Integer taskId = individualTasks.remove(uuid);
        if (taskId != null) {
    ...
    public void shutdown() {
        for (Integer taskId : individualTasks.values()) {
            Bukkit.getScheduler().cancelTask(taskId);
        }
